...
\title{\vspace{-12mm}
  ...
  \normalsize ―― シミュレータだけで出した一次結果と、そこで見つかった実験設計の誤り\\[1mm]}
\author{\normalsize 児玉靖司}
...
\keyp{$-10\,\%$ から $-34\,\%$ へ単調に強まり、$\pm 50$\,ms 以上では5種すべてで齢ありが勝つ}。
すなわち\keyp{齢が効くのは遅延が大きいときではなく、遅延がばらつくときである}。
一方\prob{強化学習側では両実験とも齢の効果が検出できず}、
...
\code{test\_planE.py} が機械的に検査する（12 項目、\keyp{全通過}）。
...
\centering
\small
\caption{実験2の結果。rms（deg）の中央値（5 種）。$d=100$\,ms 固定}
\label{tab:e2}
\begin{tabular}{@{}r rr rr rr@{}}
\toprule
...
\textbf{P2 RL・齢なし} & \textbf{P3 RL・齢あり} \\
\midrule
0\,ms   & 0.092 & 6.325 & 1.595 & \textbf{1.436} & 1.725 & 1.858 \\
25\,ms  & 0.092 & 6.391 & 1.782 & \textbf{1.627} & 1.967 & 2.038 \\
50\,ms  & 0.092 & 6.561 & 2.083 & \textbf{1.740} & 2.358 & 2.503 \\
100\,ms & 0.092 & 6.944 & 2.942 & \textbf{2.202} & 3.316 & 2.990 \\
200\,ms & 0.092 & 7.778 & 4.198 & \textbf{2.765} & 3.612 & 4.191 \\
\bottomrule
\end{tabular}
\end{table}
...
\addplot[very thick,red!70!black,mark=*,mark size=1.6] coordinates
  {(0,1.436)(25,1.627)(50,1.740)(100,2.202)(200,2.765)};
\addlegendentry{P1 模倣・齢あり}
\addplot[thick,blue!45!black,mark=triangle,mark size=2.2] coordinates
  {(0,1.725)(25,1.967)(50,2.358)(100,3.316)(200,3.612)};
\addlegendentry{P2 RL・齢なし}
\addplot[thick,blue!70!green,mark=triangle*,mark size=2.2] coordinates
  {(0,1.858)(25,2.038)(50,2.503)(100,2.990)(200,4.191)};
\addlegendentry{P3 RL・齢あり}
\end{axis}
\end{tikzpicture}
...
\midrule
0\,ms   & $-10.0\,\%$ & 4/5 & $+7.7\,\%$ & 2/5 \\
25\,ms  & $-8.7\,\%$  & 4/5 & $+3.6\,\%$ & 1/5 \\
50\,ms  & $-16.4\,\%$ & \keyp{5/5} & $+6.2\,\%$ & 1/5 \\
100\,ms & $-25.1\,\%$ & \keyp{5/5} & $-9.8\,\%$ & 3/5 \\
200\,ms & \keyp{$-34.1\,\%$} & \keyp{5/5} & $+16.0\,\%$ & 1/5 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=1.8em,itemsep=4pt]
  \item \keyp{予測1は的中した。} 齢の効果は $-10\,\% \to -34\,\%$ と\keyp{単調に強まり}、
        $j\ge 50$\,ms では\keyp{5 種すべてで}齢ありが勝つ。
        種ごとの差も $-0.05$\,deg 級から $-1.8$\,deg 級へ一桁大きくなる。
  ...
  \item \prob{予測3は外れ、強化学習では今回も齢の効果が出なかった}（勝ち 1/5〜3/5、符号も一定しない）。
        実験1と合わせて、\keyp{CEM が齢を使う解に届いていない}疑いが濃い。
        121 次元の分布探索で、8 本ある入力のうち 2 本の使い方だけを見つけるのは、
        勾配を使わない探索には荷が重いという読みが自然である。
        別のアルゴリズムでの確認が要る。
  \item \keyp{予測2は検定できなかった。} 本実験は $a$ と $\hat a$ を\prob{同時に}与えており、
        両者を分離していない。どちらがどれだけ効いているかは
        アブレーション（$a$ のみ／$\hat a$ のみ／両方）でしか言えない。
        \prob{これは実験計画の不足であり、次の実験の第一項目にする。}
\end{enumerate}
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt]
  ...
  \item \prob{$a$ と $\hat a$ を分離していない}。両方を同時に与えているので、
        案Eに固有の入力である $\hat a$ が単独でどれだけ効くかは\keyp{まだ言えない}。
        アブレーションが要る（第\ref{sec:next}節の第1項）。
...
\begin{enumerate}[leftmargin=1.8em,itemsep=3pt]
  \item \keyp{$a$ と $\hat a$ のアブレーション}（4 条件：どちらも無し／$a$ のみ／$\hat a$ のみ／両方）。
        案Eに固有の入力が単独で効くかは、これでしか言えない。\keyp{最優先}。
  \item 脱落率 \code{p\_drop}（メールボックス溢れ、\code{cc.c:532}）の軸追加。
        \keyp{$\hat a$ が外れる条件で何が起きるか}が本題になる。
        $\hat a$ を「点推定」でなく「上界」として渡すべきかの判断がここで決まる。
...
\begin{enumerate}[leftmargin=1.8em,itemsep=3pt]
  ...
  \item \keyp{齢の効果の主戦場は大遅延ではなく高ジッタである。}
        したがって案Dの合格ラインも、遅延の大きさではなくばらつきの関数として書くべきである。
        実験2で得た $-34\,\%$（$\pm 200$\,ms、5/5 種）が、その最初の目安になる。
\end{enumerate}
...
\code{test\_planE.py} & 実験系そのものの検証（12 項目） \\
