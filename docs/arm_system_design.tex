...
\toprule
\textbf{案} & \textbf{構成} & \textbf{評価}\\
\midrule
A & 6 関節 $\times$ 1 台 & アーム全軸が実機 Xinu になる。ただし\prob{冗長性がゼロ}で、
    当初構想の中核であるフェイルオーバとアクター移動を\textbf{実証できない}\\
B & \good{3 関節 $\times$ 2 台} & 3 つの独立したリソースで冗長化が成立することを示せる。
    残る 3 関節は CG 側の soft-Xinu が担う\\
\bottomrule
\end{tabularx}
\end{center}

\good{案 B を採る。} 理由は、実証したい主張が「アームが動くこと」ではなく
「\textbf{Capability に基づく役割分担・冗長化・動的移動が成立すること}」だからである。
案 A では冗長性が 1 つも無く、リース移譲も昇格も試験できない。
一方、案 B で足りない 3 関節は CG が肩代わりするので、
\textbf{アームとしては 6 軸すべてが動く}。ここがシミュレータを持っていることの利点である。

実機に載せる 3 関節は \textbf{ID1（ベース旋回）・ID2（肩）・ID3（肘）} とする。
この 3 つが搬送時の関節変位の大半を占め、サイクル時間を支配するため、
実時間性の評価対象として意味がある。ID4--ID6 は CG 側に残す。
...
\midrule
1 & ID1 & \code{ServoMotor}（主） & \code{!\{act, mut, net\}}\\
2 & ID1 & \code{ServoSensor}（副） & \code{!\{net\}}\\
3 & ID2 & \code{ServoMotor}（主） & \code{!\{act, mut, net\}}\\
4 & ID2 & \code{ServoSensor}（副） & \code{!\{net\}}\\
5 & ID3 & \code{ServoMotor}（主） & \code{!\{act, mut, net\}}\\
6 & ID3 & \code{ServoSensor}（副） & \code{!\{net\}}\\
\bottomrule
\end{tabular}
\end{center}

\textbf{台数効果の測定点は 2 / 4 / 6 の 3 点}になる。1 関節対・2 関節対・3 関節対と
段階的に増やせば、同じ作業に対するサイクル時間の傾向が取れる。
1 点では傾向を主張できないので、\textbf{6 台という数は「3 点が取れる最小」}でもある。

\textbf{純粋アクター（\code{Kinematics}）は 6 台すべてに複製する。}
効果を持たないので二重実行が無害であり、冗長化の費用がゼロだからである
（第~\ref{sec:rules} 節）。
...
\section{冗長化 --- 3 関節 $\times$ 2 台}

\subsection{対の構造}

実機を載せる 3 関節それぞれに 2 台を割り当てる。当初構想どおりだが、
役割を「モータ役とセンサ役」ではなく\textbf{「\code{act} 保持者と待機者」}として定義する。

\begin{center}\small
\begin{tabularx}{\textwidth}{@{}l l X@{}}
\toprule
\textbf{ノード} & \textbf{保持} & \textbf{役割}\\
\midrule
Xinu-A（主） & \code{ServoMotor}\,\code{!\{act,mut,net\}} & 目標角を受けてサーボを駆動\\
Xinu-B（副） & \code{ServoSensor}\,\code{!\{net\}} & 同じ関節を計測し、\textbf{主の状態を鏡像として保持}\\
\bottomrule
\end{tabularx}
\end{center}

副が主の状態を常に持っている点が要である。\textbf{昇格時にネットワーク越しの
状態転送が要らない。} 今日の実測ではメッシュの裾が秒単位あるため、
「故障を検知してから状態を取りに行く」設計は成立しない。

\subsection{昇格 = Capability の移譲}

\code{act} は厳密に 1 つという規則があるので、昇格は\textbf{排他的な移譲}でなければ
ならない。分断時に主副が同時に駆動する事態は物理的に危険である。
そこで \code{act} を\textbf{期限付きリース}として扱う。

\begin{enumerate}[leftmargin=1.6em,itemsep=3pt]
\item 主は周期 $T_r$ でリースを更新する。
\item 主が $T_e$ 以内に更新できなければ、\textbf{主は自力で駆動を停止する}
      （ネットワーク不要。ローカルの時計だけで判定できる）。
\item 副は $T_e$ 経過を観測してから \code{act} を取得し、昇格する。
\end{enumerate}

\prob{手順 2 が安全性の要である。} 分断された主が「自分はまだ主だ」と思い込んで
駆動し続けることを、ネットワークに頼らず防ぐ。これは分散ロックのリースと同じ論法で、
...
\midrule
\code{ServoMotor} & 10\,Hz & 硬 & \textbf{RT スケジューラを持つノード}\\
\code{ServoSensor} & 10\,Hz & 軟 & 同上（対の相方）\\
