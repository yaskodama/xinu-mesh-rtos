...
コード転送 \code{/actor/load} & \good{38--115\,ms} & 実測(2026-07-21)。Pi 5 中央値 37.9\,ms /
  Pi 4 114.8\,ms。1743\,B の \code{ServoMotor} を投入し \texttt{1 actor live} を確認\\
状態の読み直し & $\sim$1 往復 & \code{Arm\_serial\_servo\_read}\\
\midrule
\prob{フェイルオーバ所要} & \prob{3--5\,s} & 上記の合計。\good{コード転送は実測で
  0.04--0.12\,s にすぎず}、支配するのはリース期限 $T_e$ である\\
\bottomrule
\end{tabular}
\end{center}

\prob{したがってこの冗長化は「可用性」のためであって「無停止」のためではない。}
3--5 秒のあいだ、その軸には駆動権限を持つ者がいない。

\textbf{当初 4--8 秒と見積もったが、実測でコード転送が安いことが分かり 3--5 秒に下がった。}
\code{/actor/load} の所要時間は Pi 5 で中央値 37.9\,ms、Pi 4 で 114.8\,ms
（1743\,B の \code{ServoMotor} を投入し常駐を確認）。しかもその大半は JIT ではなく
HTTP 自体の遅延で、Pi 5 の通常 HTTP が 38\,ms、Pi 4 が 130\,ms であることと符合する。
\good{コード移動の費用は無視できる} --- 転送方式の弱点は転送そのものではなく、
故障検知に要するリース期限である。
...
1 & \code{act} 効果の分離（\code{Arm\_serial\_servo\_write} を \code{mut} から移す） &
    \good{完了}: \code{--type-check} が駆動する 3 メソッドだけを検出し、
    \code{ServoMotor} と \code{ShelfActor} が型で分かれた\\
2 & 配置規則の実装（効果集合 → 配置可能ノード集合） & 単体テスト\\
3 & 状態の直列化（アクターのフィールドを運ぶ） & 移動前後で状態が一致\\
4 & リース機構（\code{act} の排他移譲） & 分断注入で二重駆動が起きないこと\\
4b & \code{/actor/load} の所要時間測定 & \good{完了}: Pi 5 37.9\,ms / Pi 4 114.8\,ms\\
...
\item リースの $T_r$ / $T_e$ は実測レイテンシからの\textbf{見積もり}であり、
      分断注入試験で検証していない。
\item \code{act} 分離は\good{実装済}（aios-claude \code{25b7c02}、
      aipl-line-simulator \code{bad8ab9}）。\code{--type-check} は
      \code{[type] no issues}、\code{test/aipl\_sync.mjs} も申告漏れ 0 件。
...
\item \code{/actor/load} は\good{実測済}（Pi 5 37.9\,ms / Pi 4 114.8\,ms、各 20 回）。
      ただしリースの $T_r$ / $T_e$ は依然として見積もりで、分断注入試験をしていない。
